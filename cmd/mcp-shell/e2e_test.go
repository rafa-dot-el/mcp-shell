...
func TestE2EPerformance(t *testing.T) {
	...
	if exitCode != 0 {
		t.Errorf("Performance test command failed with exit code %d. Stderr: %s", exitCode, stderr)
	}

	// Check startup time (should be reasonably fast)
	maxStartupTime := 5 * time.Second
	if duration > maxStartupTime {
		t.Errorf("Command took too long to execute: %v (max: %v)", duration, maxStartupTime)
	}

	// Check output contains version info
	if !strings.Contains(stdout, "version") {
		t.Errorf("Performance test didn't produce expected output: %s", stdout)
	}

	t.Logf("Command startup time: %v", duration)
}

// TestE2EMemoryUsage tests basic memory usage (if monitoring tools are available).
func TestE2EMemoryUsage(t *testing.T) {
	...
	// Run a simple command and check it completes without issues
	// In a more sophisticated test, you might use tools like valgrind,
	// or monitor memory usage programmatically
	stdout, stderr, exitCode := runCommand(t, binaryPath, "version")

	if exitCode != 0 {
		t.Errorf("Memory usage test command failed with exit code %d. Stderr: %s", exitCode, stderr)
	}

	if !strings.Contains(stdout, "version") {
		t.Errorf("Memory usage test didn't produce expected output: %s", stdout)
	}

	// This is a basic test - in a real scenario, you might:
	// - Monitor RSS/VSZ memory usage
	// - Check for memory leaks
	// - Test with large inputs
	// - Use specialized tools for memory profiling

	t.Log("Basic memory usage test completed successfully")
}

// TestE2EConcurrentExecution tests multiple concurrent executions.
func TestE2EConcurrentExecution(t *testing.T) {
	if testing.Short() {
	...
	for i := 0; i < numConcurrent; i++ {
		go func(id int) {
			...
			if exitCode != 0 {
				results <- fmt.Errorf("concurrent execution %d failed with exit code %d, stderr: %s", id, exitCode, stderr)
				return
			}

			if !strings.Contains(stdout, "version") {
				results <- fmt.Errorf("concurrent execution %d produced unexpected output: %s", id, stdout)
				return
			}
