...
The Docker configuration uses three volume mounts:

1. **Config mount** (`~/.config/mcp-shell:/config:ro`):
   - Mounts your MCP Shell configuration into the container
   - Read-only (`:ro`) for security
   - Configuration file is read from `/config/config.yaml` inside container

2. **Log mount** (`~/.local/state/mcp-shell:/logs`):
   - Mounts log directory for execution logs
   - Read-write to allow log file creation
   - Logs persist on host after container stops

3. **Workspace mount** (`${PWD}:/workspace`):
   - Mounts current working directory into container
   - Allows scripts to access files in your project
   - Working directory is set to `/workspace` in container
...
# Specific version
ghcr.io/rafa-dot-el/mcp-shell:v1.0.0

# Development branch
ghcr.io/rafa-dot-el/mcp-shell:dev
```

### Testing Docker Configuration

Test the Docker setup before using with Claude:
...
ls -l ~/.config/mcp-shell/config.yaml
```

**Volume mount issues:**
```bash
# Use absolute paths instead of ~ if needed
{
  "mcpServers": {
    "mcp-shell": {
      "command": "docker",
      "args": [
        ...
        "--rm",
        "-i",
        "-v", "/home/user/.config/mcp-shell:/config:ro",
        "-v", "/home/user/.local/state/mcp-shell:/logs",
        "-v", "${PWD}:/workspace",
        "-w", "/workspace",
        "ghcr.io/rafa-dot-el/mcp-shell:latest",
        "serve"
...
- [Claude Code Tutorial](/tutorials/claude-code/): Use MCP Shell with Claude Code CLI
